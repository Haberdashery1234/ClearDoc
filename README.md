...
`.github/workflows/ci.yml` builds and tests both schemes on GitHub's
`macos-26` runner against an iOS Simulator, on every push/PR to `main`. The
disabled model-calling tests simply report as skipped in CI, which is
expected — there's no real device, signed-in Apple Intelligence, or
downloaded model available on a CI runner regardless.
